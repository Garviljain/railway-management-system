...
# Create a notebook (tabbed layout) for "Available Trains" and "User" functionalities
notebook = ttk.Notebook(root)

# Add tabs to the notebook for "Available Trains" and "User" functionalities
tab_available_trains = ttk.Frame(notebook)
tab_user = ttk.Frame(notebook)

notebook.add(tab_available_trains, text="Available Trains")
notebook.add(tab_user, text="User")
notebook.grid(row=0, column=0, columnspan=2, padx=10, pady=10, sticky="nsew")
notebook.pack(expand=True, fill="both")

# Create labels and entry fields for "Available Trains" tab
label_start_station = tk.Label(tab_available_trains, text="From Station Name:")
label_start_station.grid(row=0, column=0, padx=5, pady=5)

entry_start_station = tk.Entry(tab_available_trains)
entry_start_station.grid(row=0, column=1, padx=5, pady=5)

label_final_station = tk.Label(tab_available_trains, text="To Station Name:")
label_final_station.grid(row=1, column=0, padx=5, pady=5)

entry_final_station = tk.Entry(tab_available_trains)
entry_final_station.grid(row=1, column=1, padx=5, pady=5)

label_date = tk.Label(tab_available_trains, text="Date (YYYY-MM-DD):")
label_date.grid(row=2, column=0, padx=5, pady=5)

entry_date = tk.Entry(tab_available_trains)
entry_date.grid(row=2, column=1, padx=5, pady=5)

# Button to search for available trains
button_search = tk.Button(tab_available_trains, text="Search", command=available_trains)
button_search.grid(row=3, column=0, columnspan=2, padx=5, pady=5)

# Treeview to display results
treeview = ttk.Treeview(tab_available_trains, columns=("Train No", "Source Station", "Destination Station", "Arrival Time", "Departure Time"))
...
label_result = tk.Label(tab_available_trains, textvariable=result_text)
label_result.grid(row=5, column=0, columnspan=2, padx=5, pady=5)

# Button to open book train window
button_book_train = tk.Button(tab_user, text="Book Train", command=open_book_train_window)
button_book_train.pack(padx=5, pady=5)

# Button to open show bookings window
button_show_bookings = tk.Button(tab_user, text="Show Bookings", command=open_show_bookings_window)
button_show_bookings.pack(padx=5, pady=5)

# Button to open cancel booking window
button_cancel_booking = tk.Button(tab_user, text="Cancel Booking", command=open_cancel_booking_window)
button_cancel_booking.pack(padx=5, pady=5)

# Button to check fare
button_check_fare = tk.Button(tab_user, text="Check Fare", command=check_fare)
button_check_fare.pack(padx=5, pady=5)

# Text widget to display fare
fare_text = tk.Text(tab_user, height=10, width=50)
fare_text.pack(padx=5, pady=5)
